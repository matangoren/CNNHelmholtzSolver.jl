...
function check_model!(test_name, model, n, m, h, kappa, omega, gamma, e_vcycle_input, kappa_type, kappa_input, gamma_input, kernel, dataset_size, restrt, max_iter; v2_iter=10, level=3, smooth=false, k_kernel=3, threshold=50, axb=false, norm_input=false, before_jacobi=false, log_error=false, unet_in_vcycle=false, indexes=3, arch=1) # arch - Standart (0) SplitUNet (1) FeaturesUNet (2)
    ...
    for i=1:dataset_size
        ...
        heatmap(kappa, color=:blues)
        savefig("test/unet/results/kappa $(kappa_type) $(threshold) $(k_kernel) $(i)")
        # figure(); imshow(x, cmap=:jet);colorbar(); clim(1.5,4.5); PyPlot.savefig("test/unet/results/gg")
        CSV.write("test/unet/results/kappa $(kappa_type) $(threshold) $(k_kernel) $(i).csv", DataFrame(K=vec(kappa)), delim = ';')
        kappa_features = NaN
        if arch != 0
        ...
        println("Before 1")
        resvec1, resvec2, x1, x2 = unet_vs_vcycle_cpu_gmres!(model, n, m, h, kappa, kappa_features, omega, gamma, x_true, true, e_vcycle_input, kappa_input, gamma_input, restrt, max_iter, false; v2_iter=v2_iter, level=level, axb=axb, norm_input=norm_input, log_error=log_error, test_name=test_name, before_jacobi=before_jacobi, unet_in_vcycle=unet_in_vcycle, arch=arch)
        unet_results[i,1,:] = resvec1
        vcycle_results[i,1,:] = resvec2
        heatmap(real(reshape(x1[:,1],n+1,m+1)), color=:blues)
        savefig("test/unet/results/VU_unet_pre_linear")
        heatmap(real(reshape(x2[:,1],n+1,m+1)), color=:blues)
        savefig("test/unet/results/VU_vcycle_pre_linear")
        println("After 1")
        resvec1, resvec2, x1, x2 = unet_vs_vcycle_cpu_gmres!(model, n, m, h, kappa, kappa_features, omega, gamma, x_true, false, e_vcycle_input, kappa_input, gamma_input, restrt, max_iter, false; v2_iter=v2_iter, level=level, axb=axb, norm_input=norm_input, log_error=log_error, test_name=test_name, before_jacobi=before_jacobi, unet_in_vcycle=unet_in_vcycle, arch=arch)
...
function check_point_source_problem!(test_name, model, n, m, h, kappa, omega, gamma, e_vcycle_input, kappa_input, gamma_input; v2_iter=50, level=3)
    b = zeros(c_type,n+1,m+1)|>pu

    b[floor(Int32,n / 2.0),floor(Int32,m / 2.0)] = 1.0 ./ minimum(h)^2;
    ...
    input = complex_grid_to_channels!(reshape(b , n+1, m+1, 1, 1))
    if arch == 1
        kappa_features = model.kappa_subnet(reshape(kappa.^2, n+1, m+1, 1, 1)|>cgpu)|>pu
    ...
    e_unet = coefficient .* e_unet
    heatmap(real(e_unet[:,:,1,1])|>cpu, color=:blues)
...
function unet_vs_vcycle_cpu_gmres!(model, n, m, h, kappa, kappa_features, omega, gamma, x_true, after_vcycle, e_vcycle_input, kappa_input, gamma_input, restrt, max_iter, relax_jacobi; v2_iter=10, level=3, axb=false, norm_input=false, log_error=true, test_name="", before_jacobi=false, unet_in_vcycle=false, arch=1)
    ...
    return cat([resvec3[end]],resvec1, dims=1), cat([resvec3[end]],resvec2,dims=1), x1, x2
end
