...
function gpu_flexible_gmres(A::Function,b,restrt; tol,maxIter,M::Function,x,out::Int=0,storeInterm::Bool=false,flexible::Bool=false,mem::FGMRESmem =  getEmptyFGMRESmem())
		...
		if norm(b)==0.0
			return zeros(eltype(b),n)|>gpu,-9,0.0,0,[0.0];
		end
		...
		while iter < maxIter
			...
			for j = 1:restrt
				...
				# Gram Schmidt (much faster than MGS even though zeros are multiplied, does relatively well):
				# d_vw = Dense(V', zeros(r_type, size(V',1)))|>gpu
				# d_vw = Dense(V', true)|>gpu
				# t = d_vw(w) # BLAS.gemv!('C', constOne, V, w,constZero,t);# t = V'*w;
				t = V'*w;
				t[j+1:end] .= r_type(0.0);
				H[1:restrt,j] = t;

				# d_vt = Dense(-V, w)|>gpu
				# w = d_vt(t) # BLAS.gemv!('N', -constOne, V, t,constOne,w);# w = w - V*t;
				w = w - V*t;
				betta = r_type(norm(w));
				H[j+1,j] = betta;
			...
			# pin_h = pinv(H)
			# d_hx = Dense(pin_h, zeros(r_type, size(pin_h, 1)))|>gpu
			# d_hx = Dense(pin_h, true)|>gpu
			# y = d_hx(xi)
			y = pinv(H)*xi

			if flexible
				# d_zy = Dense(Z|>cpu, zeros(r_type, size(Z, 1)))|>gpu
				# d_zy = Dense(Z|>cpu, true)|>gpu
				# w = d_zy(y) #BLAS.gemv!('N', constOne, Z, y,constZero,w); # This is the correction that corresponds to the residual.
				w = Z*y;
			else
