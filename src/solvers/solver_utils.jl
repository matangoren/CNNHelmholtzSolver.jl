...
function solve(param::CnnHelmholtzSolver, r_vcycle, restrt, max_iter; v2_iter=10, level=3, axb=false)
    ...
    return reshape(x1,(n+1)*(m+1),blocks)
...
function retrain_model(param, model, base_model_folder, new_model_name, n, m, h, kappa, omega, gamma,
    ...
    for iteration in 1:iterations
        ...
        for (batch_r, batch_e) in data_loader
            ...
            # r_model = helmholtz_chain!(e_model, helmholtz_matrix; h=h) # Ae_model
            ...
            r_tilde = helmholtz_chain!(e_tilde, helmholtz_matrix; h=h) .* coefficient # Ae_tilde
