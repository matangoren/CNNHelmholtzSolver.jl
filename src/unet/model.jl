...
struct UNetUpBlock
  ...
  UNetUpBlock(in_chs::Int, out_chs::Int; stride=(2,2), pad=2, kernel = (5, 5), p = 0.5f0, σ=elu) =
...
function FFSDNUnet(channels::Int = 2, labels::Int = channels; kernel = (3, 3), σ=elu, resnet_type=SResidualBlock)
    conv_down_blocks = Chain(ConvDown(channels,16;σ=σ),
            ConvDown(32,32;σ=σ),
            ConvDown(64,64;σ=σ),
            ConvDown(128,128; stride=(1,1), σ=σ))|> cgpu
...
function (u::FFSDNUnet)(x::AbstractArray, features)
    ...
    # (n/8) X (n/8) X 64 X bs -> (n/8) X (n/8) X 128 X bs
    ...
    up_x1 = u.up_blocks[1](cat(up_x3, features[4], dims=3), x1)
    # (n/4) X (n/4) X 128 X bs -> (n/2) X (n/2) X 32 X bs
    up_op = u.up_blocks[2](cat(up_x1, features[5], dims=3), op)
    # (n/2) X (n/2) X 32 X bs -> (n/2) X (n/2) X 16 X bs
    up_x = u.up_blocks[3](cat(up_op, features[6], dims=3))

    # (n/2) X (n/2) X 16 X bs -> n X n X 2 X bs
    return u.up_blocks[4](up_x)
end
...
function TFFKappa(channels::Int = 2, labels::Int = channels; kernel = (3, 3), σ=elu, resnet_type=SResidualBlock)
    conv_down_blocks = Chain(
            ConvDown(channels,16;σ=σ),
            ConvDown(16,32;σ=σ),
            ConvDown(32,64;σ=σ),
            ConvDown(64,128;stride=(1,1),σ=σ))|> cgpu # changed last layer to be without downsample
...
function (u::TFFKappa)(x::AbstractArray)
    ...
    up_x3 = u.conv_blocks[13](up_x3)

    # (n/8) X (n/8) X 128 X bs -> (n/4) X (n/4) X 64 X bs
    up_x1 = u.conv_blocks[12](u.up_blocks[1](up_x3, x1))
    # (n/4) X (n/4) X 128 X bs -> (n/2) X (n/2) X 32 X bs
    up_op = u.conv_blocks[11](u.up_blocks[2](up_x1, op))

    return [op, x1, x2, up_x3, up_x1, up_op]
end
